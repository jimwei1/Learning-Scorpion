...
class SOLUTION:
    ...
    def Create_Brain(self):
        ...
        for leg in range(self.numofLegs):
            ...
            lastLinkNum = self.numofLinksDict[leg] - 1
            lastLinkName = str(leg) + "Link" + str(lastLinkNum)
            firstJointName = "Torso_" + firstLinkName
            pyrosim.Send_Sensor_Neuron(name = self.counter, linkName = lastLinkName)
            sensorNeuron = self.counter
            pyrosim.Send_Motor_Neuron(name = self.counter , jointName = firstJointName)
            motorNeuron = self.counter
